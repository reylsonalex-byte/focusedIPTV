  ...
  useEffect(() => {
    ...
    const enableRotation = async () => {
      try {
        ...
        console.log('Erro ao liberar rotação no player:', e);
      }
    };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!nextStream) {
          if (!contentId) throw new Error('Id inválido');
          ...
          nextMeta = {
            ...
            duration:
              displayValue(movie.duration) || displayValue(info.duration),
            genre: cleanString(movie.genre) || cleanString(info.genre),
          };
        ...
        if (!nextStream) throw new Error('Stream indisponível');
        if (cancelled) return;
        ...
        if (!cancelled) setError('Não foi possível carregar este conteúdo.');
      } finally {
    ...
    return () => {
      ...
      if (historyKey) {
        void updateProgress(
          historyKey,
          Math.max(0, Math.floor(resumeTimeRef.current || 0)),
          durationSec
        );
      }
    };
  }, [contentId, directStreamUrl, durationSec, historyKey, initialTitle, router]);

  const metaLine = useMemo(() => {
    return [meta.year, meta.rating, meta.duration, meta.genre]
      .filter(Boolean)
      .join(' • ');
  }, [meta]);
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        <View style={styles.center}>
          ...
          <Text style={styles.loadingText}>Preparando player premium...</Text>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.hero}>
            <LinearGradient
              colors={['rgba(5,5,7,0.55)', 'rgba(5,5,7,0.80)', '#09090C']}
            ...
            >
              <View style={styles.topRow}>
                ...
                <View style={styles.resumePill}>
                  <Ionicons name="phone-landscape-outline" size={14} color="#E9D5FF" />
                  <Text style={styles.resumeText}>Rotação automática ativa</Text>
                </View>
              </View>
              ...
              <View style={styles.playerShell}>
                {streamUrl ? (
                  <Video
                    ...
                    onLoad={(data: any) => {
                      setPlayerReady(true);
                      setDurationSec(Number(data?.duration || 0));

                      if (!hasSeekedRef.current && resumeTimeRef.current > 0) {
                    ...
                    onProgress={(data: any) => {
                      ...
                      if (currentTime > 0) {
                        void updateProgress(
                          historyKey,
                          currentTime,
                          durationSec || Number(data?.playableDuration || 0)
                        );
                      }
                    }}
                    onError={(e: any) => {
                      console.log('react-native-video error', e);
            ...
            <View style={styles.headerBlock}>
              ...
              <View style={styles.headerInfo}>
                <View style={styles.tag}>
                  <Text style={styles.tagText}>HBO MAX STYLE</Text>
                </View>
                ...
                <View style={styles.quickStats}>
                  <View style={styles.quickStatCard}>
                    <Ionicons name="play-circle-outline" size={18} color="#C4B5FD" />
                  ...
                  <View style={styles.quickStatCard}>
                    ...
                    <Text style={styles.quickStatText}>
                      {resumeSeconds > 0 ? formatClock(resumeSeconds) : 'Novo'}
                    </Text>
                  </View>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  video: { width: '100%', height: '100%', backgroundColor: '#141418' },
  detailsWrap: { paddingHorizontal: 16, marginTop: 8 },
  ...
  quickStatText: { color: '#FFF', fontSize: 12, fontWeight: '700' },
  errorCard: {
    marginTop: 18,
    borderRadius: 18,
    padding: 14,
  ...
  plot: { color: '#B6B6C3', fontSize: 15, lineHeight: 24 },
});    stream_icon?: string;
    container_extension?: string;
    direct_source?: string;
  };
